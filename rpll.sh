...
configure_graphlet_mode() {
    case "$GRAPHLET_K" in
        ...
        5)
            GRAPHLET_DIM=21
            # onb 5 emits these 21 graphlet IDs in this order.
...
if (( ! INITIAL_GRAPH_GIVEN )); then
    ...
    ./onb 4 synth_base_1.txt > bres.tmp
    cat data_middle.txt > data_middle1.txt
    invoke_expfy "$TARGET" synth_base_1.txt "$INIT_PRIMARY_SAMPLES" "$INIT_PRIMARY_SAMPLES" 1 8 20 "$INIT_HDGB_1" > synth_base_2.txt
    ./onb 4 synth_base_2.txt > bres.tmp
    cat data_middle.txt > data_middle1.txt
    invoke_expfy "$TARGET" synth_base_2.txt "$INIT_MAIN_SAMPLES" "$INIT_MAIN_SAMPLES" 1 8 18 "$INIT_HDGB_2" > synth_base_3.txt
    ./onb 4 synth_base_3.txt > bres.tmp
    cat data_middle.txt > data_middle1.txt
    invoke_expfy "$TARGET" synth_base_3.txt "$INIT_MAIN_SAMPLES" "$INIT_MAIN_SAMPLES" 1 8 18 "$INIT_HDGB_3" > synth_base_4.txt
    ./onb 4 synth_base_4.txt > bres.tmp
    cat data_middle.txt > data_middle1.txt
    invoke_expfy "$TARGET" synth_base_4.txt "$INIT_MAIN_SAMPLES" "$INIT_MAIN_SAMPLES" 1 8 17 "$INIT_HDGB_4" > synth_base_5.txt
    ./onb 4 synth_base_5.txt > bres.tmp
    cat data_middle.txt > data_middle1.txt
    invoke_expfy "$TARGET" synth_base_5.txt "$INIT_MAIN_SAMPLES" "$INIT_SECONDARY_SAMPLES" 1 17 16 "$INIT_HDGB_5" > synth_base_6.txt
    ./onb 4 synth_base_6.txt > bres.tmp
    cat data_middle.txt > data_middle1.txt
    invoke_expfy "$TARGET" synth_base_6.txt "$INIT_MAIN_SAMPLES" "$INIT_SECONDARY_SAMPLES" 1 17 15 "$INIT_HDGB_6" > synth_base_7.txt
    ./onb 4 synth_base_7.txt > bres.tmp
    cat data_middle.txt > data_middle1.txt
    invoke_expfy "$TARGET" synth_base_7.txt "$INIT_MAIN_SAMPLES" "$INIT_SECONDARY_SAMPLES" 1 17 14 "$INIT_HDGB_7" > synth_base_8.txt
...
pis_graph_rseas() {
    ...
    if ! ./onb "$GRAPHLET_K" "$GRAPH" > "$VEC_FILE"; then
...
./onb "$GRAPHLET_K" "$TARGET" > "$PIS_TARGET_VEC"
./onb "$GRAPHLET_K" "$PRE_INIT_SYNTH" > "$local_pis_initial_vec"
...
# five-node graphlet, so it cannot be classified from onb 4's columns.
...
prepare_parallel_worker() {
    ...
    # onb invokes counting helpers through paths such as ./orca and ./otb2.
...
apply_exp() {
    ...
    if ! invoke_expfy "$TARGET" "$INPUT_FILE" "$REQUESTED_SAMPLES" 0 \
...
apply_exp2() {
    ...
    SAMPLE_FILE=$(mktemp ".expfy_samples.XXXXXX.tmp")
    if ! invoke_expfy "$TARGET" "$INPUT_FILE" \
...
run_round() {
    ...
    (
        ...
        local ONB_ABS EVS_ABS BXK4F_ABS BXK4ONE_ABS
        ...
        "$ONB_ABS" "$GRAPHLET_K" "$INPUT_ABS" > "$VEC_INS"
        "$ONB_ABS" 4 "$INPUT_ABS" > "$ROOT_DIR/bres.tmp"
        ...
        for IDX in "${!EVC_LABELS[@]}"; do
            ...
            (
                ...
                if [ "$GATE_STATUS" = "DISALLOWED" ]; then
                    ...
                    # rejection and skip the much more expensive onb call.
                ...
                ./onb "$GRAPHLET_K" candidate.graph > candidate.vec
        ...
        for IDX in "${!TOP_LINES[@]}"; do
            ...
            (
                ...
                ./onb "$GRAPHLET_K" "$STAGE1" > stage1.vec
                ...
                ./onb "$GRAPHLET_K" "$FINAL_CANDIDATE" > final.vec
        ...
        if float_greater "$BEST_SCORE" "$had_score"; then
            ...
            for ((REFINE_ITER=1; REFINE_ITER<=PAIR_REFINEMENT_PASSES; REFINE_ITER++)); do
                ...
                # one onb 5 call for the objective vector and one onb 4 call for
                # data_middle1.txt.
                "$ONB_ABS" "$GRAPHLET_K" "$REFINE_GRAPH" > "$CURRENT_VEC"
                if (( GRAPHLET_K == 4 )); then
                    cp "$ROOT_DIR/data_middle.txt" "$ROOT_DIR/data_middle1.txt"
                else
                    "$ONB_ABS" 4 "$REFINE_GRAPH" > "$REFINE_DIR/bres_${REFINE_ITER}.tmp"
                    cp "$ROOT_DIR/data_middle.txt" "$ROOT_DIR/data_middle1.txt"
                fi
                ...
                "$ONB_ABS" "$GRAPHLET_K" "$PROBE_GRAPH" > "$PROBE_VEC"
                ...
                "$ONB_ABS" "$GRAPHLET_K" "$REFINED_CANDIDATE" > "$CANDIDATE_VEC"
        ...
        "$ONB_ABS" "$GRAPHLET_K" "$OUTPUT_ABS" > /dev/null
    )
}
...
generate_outinp() {
        ...
        local INS_ABS TARGET_ABS VEC_INS_ABS ONB_ABS
        ...
        for VAL in "${ACTIVE_CANDS[@]}"; do
                (
                        ...
                        # expfy/onb use files in the current directory.  Each candidate gets
                        ...
                        # wrappers such as onb that launch ./orca and ./otb2 while keeping
                        # their generated data files job-local.
                        for TOOL in "$ROOT_DIR"/*; do
                        ...
                        [ -e "$WORK_DIR/onb" ] || ln -s "$ONB_ABS" "$WORK_DIR/onb"
...
# Main loop
CURRENT="$INIT_SYNTH"
./onb "$GRAPHLET_K" "$TARGET" > vecTar.txt
...
for ((i=1; i<=ROUNDS; i++)); do
    ...
    ./onb "$GRAPHLET_K" "$CURRENT" > vecInS.txt
    set_round_rseas_state "$i" vecInS.txt vecTar.txt

    if (( (i-1) % RBGO == 0 )); then
        ./onb 4 "$CURRENT" > bres.tmp
        cat data_middle.txt > data_middle1.txt
